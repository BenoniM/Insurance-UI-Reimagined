    ...
    <SectionWrapper id="giveback" className="relative">
        ...
        {steps.map((step, i) => (
          <ScrollReveal key={step.n} delay={i * 0.08}>
            <motion.div
              ...
              className="qupe-card !p-8 h-full text-center relative"
            >
        ...
        {causes.map((cause, i) => (
          <ScrollReveal key={cause.title} animation="fadeUp" delay={i * 0.06}>
            <motion.div
              ...
              transition={{ type: "spring", stiffness: 300, damping: 20 }}
              className={`rounded-3xl p-7 bg-gradient-to-br ${cause.color} text-white shadow-lg relative overflow-hidden h-full`}
            >
              <div className="absolute -top-6 -right-6 w-24 h-24 rounded-full bg-white/10" />
              <motion.div
                whileHover={{ rotate: 12, scale: 1.1 }}
                transition={{ type: "spring", stiffness: 300 }}
                className="relative z-10 w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center mb-4"
              >
      ...
      <ScrollReveal delay={0.2}>
        <motion.div
          ...
          className="mt-16 max-w-5xl mx-auto rounded-3xl p-10 md:p-12 bg-gradient-to-br from-[hsl(201,78%,23%)] to-[hsl(201,78%,18%)] text-white shadow-xl relative overflow-hidden"
        >
